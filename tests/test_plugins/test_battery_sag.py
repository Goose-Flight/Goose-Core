...
class TestBatterySagAnalyzeMotorFailure:
    ...
    def test_produces_findings(self, plugin: BatterySagPlugin, motor_failure_flight: Flight) -> None:
        findings = plugin.analyze(motor_failure_flight, {})
        assert len(findings) >= 1, "Battery plugin should produce at least one finding"

    def test_returns_finding_objects(self, plugin: BatterySagPlugin, motor_failure_flight: Flight) -> None:
        findings = plugin.analyze(motor_failure_flight, {})
        assert all(isinstance(f, Finding) for f in findings)

    def test_plugin_name_in_findings(self, plugin: BatterySagPlugin, motor_failure_flight: Flight) -> None:
        findings = plugin.analyze(motor_failure_flight, {})
        assert all(f.plugin_name == "battery_sag" for f in findings)

    def test_findings_have_valid_severity(self, plugin: BatterySagPlugin, motor_failure_flight: Flight) -> None:
        findings = plugin.analyze(motor_failure_flight, {})
    ...
    def test_findings_have_evidence(self, plugin: BatterySagPlugin, motor_failure_flight: Flight) -> None:
        findings = plugin.analyze(motor_failure_flight, {})
        for finding in findings:
...
class TestBatterySagAnalyzeNormalFlight:
    ...
    def test_produces_findings(self, plugin: BatterySagPlugin, normal_flight: Flight) -> None:
        findings = plugin.analyze(normal_flight, {})
        assert len(findings) >= 1, "Battery plugin should produce at least one finding"

    def test_no_critical_in_normal_flight(self, plugin: BatterySagPlugin, normal_flight: Flight) -> None:
        findings = plugin.analyze(normal_flight, {})
        critical_findings = [f for f in findings if f.severity == "critical"]
        assert len(critical_findings) == 0, f"Normal flight should not have critical battery findings: {[f.title for f in critical_findings]}"

    def test_returns_pass_or_warning(self, plugin: BatterySagPlugin, normal_flight: Flight) -> None:
        findings = plugin.analyze(normal_flight, {})
...
class TestBatterySagPluginInterface:
    ...
    def test_applicable_for_flights(self, plugin: BatterySagPlugin, normal_flight: Flight) -> None:
        assert plugin.applicable(normal_flight)

    def test_analyze_accepts_config_dict(self, plugin: BatterySagPlugin, normal_flight: Flight) -> None:
        findings = plugin.analyze(normal_flight, {"custom_key": "value"})
        assert isinstance(findings, list)

    def test_analyze_with_empty_config(self, plugin: BatterySagPlugin, normal_flight: Flight) -> None:
        findings = plugin.analyze(normal_flight, {})
        assert isinstance(findings, list)
