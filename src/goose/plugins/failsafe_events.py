...
class FailsafeEventsPlugin(Plugin):
    ...
    def analyze(self, flight: Flight, config: dict[str, Any]) -> list[Finding]:
        ...
        if isinstance(emergency_modes_val, str):
            emergency_modes = {m.strip().lower() for m in emergency_modes_val.split(",") if m.strip()}
        ...
        emergency_transitions = self._collect_emergency_transitions(flight.mode_changes, emergency_modes)

        total_critical = sum(1 for e in failsafe_events if e.severity == "critical")
        ...
        if total_problems == 0:
            findings.append(
                Finding(
                    ...
                    severity="pass",
                    score=100,
                    description=("No failsafe events or unexpected emergency mode transitions were detected during this flight."),
                    evidence={
                        "failsafe_count": 0,
            ...
            findings.append(
                Finding(
                    plugin_name=self.name,
                    title=(f"Failsafe events detected — {len(failsafe_events)} event(s), {total_emergency_transitions} emergency transition(s)"),
        ...
        for event in failsafe_events:
            ...
            findings.append(
                Finding(
                    ...
                    score=ev_score,
                    description=(
                        f"Failsafe event recorded at t={event.timestamp:.2f}s: {event.message}. Event type: {event.event_type}, severity: {event.severity}."
                    ),
                    evidence={
    ...
    def _collect_failsafe_events(self, events: list[FlightEvent]) -> list[FlightEvent]:
        ...
        result = []
        for event in events:
            if event.event_type == "failsafe" or event.severity == "critical" or "failsafe" in event.message.lower():
                result.append(event)
        return result
