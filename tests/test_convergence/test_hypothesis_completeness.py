...
class TestContradictingFindingsPopulated:
    def test_contradicting_findings_populated(self):
        ...
        assert len(crash_hyp.contradicting_findings) > 0, "contradicting_findings should be non-empty when PASS findings exist"
...
class TestCommunicationsLinkHypothesis:
    def test_communications_link_hypothesis_generated(self):
        ...
        assert "communications / link issue" in categories, f"Expected 'communications / link issue' hypothesis. Got: {categories}"
...
class TestImpactDamageHypothesis:
    def test_impact_damage_hypothesis_generated(self):
        ...
        hypotheses = generate_hypotheses(findings, RUN_ID)
        categories = {h.category for h in hypotheses}
        assert "impact / damage class" in categories, f"Expected 'impact / damage class' hypothesis. Got: {categories}"
...
class TestUnknownMixedWhenNoStrongTheme:
    def test_unknown_mixed_on_empty_findings(self):
        ...
        categories = {h.category for h in hypotheses}
        assert "unknown / mixed-factor event" in categories, f"Expected 'unknown / mixed-factor event'. Got: {categories}"
    ...
    def test_unknown_mixed_on_all_pass_findings(self):
        ...
        hypotheses = generate_hypotheses(findings, RUN_ID)
        themes = {h.theme for h in hypotheses}
        assert "unknown_mixed" in themes, f"Expected 'unknown_mixed' theme. Got: {themes}"
    ...
    def test_unknown_mixed_not_emitted_when_strong_theme_exists(self):
        ...
        hypotheses = generate_hypotheses(findings, RUN_ID)
        themes = {h.theme for h in hypotheses}
        assert "unknown_mixed" not in themes, "unknown_mixed should not appear when a confident theme exists"
...
class TestPluginStreamMapGone:
    def test_plugin_stream_map_not_in_canonical(self):
        ...
        assert not hasattr(canonical_mod, "_PLUGIN_STREAM_MAP"), "_PLUGIN_STREAM_MAP should have been removed from canonical.py (Task D)"
...
class TestPrimaryStreamOnAllPluginManifests:
    def test_all_registered_plugins_have_primary_stream(self):
        ...
        missing = [pid for pid, plugin in PLUGIN_REGISTRY.items() if not plugin.manifest.primary_stream]
        assert missing == [], f"These plugins are missing primary_stream on their manifest: {missing}"

    def test_primary_stream_is_string(self):
        """primary_stream must be a str on every manifest."""
        from goose.plugins import PLUGIN_REGISTRY

        non_str = [pid for pid, plugin in PLUGIN_REGISTRY.items() if not isinstance(plugin.manifest.primary_stream, str)]
        assert non_str == [], f"primary_stream must be a str, got non-str for: {non_str}"
