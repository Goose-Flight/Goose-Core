...
class TestCrashDetectionVibrationCrash:
    ...
    def test_detects_crash(
        self, plugin: CrashDetectionPlugin, vibration_crash_flight: Flight
    ) -> None:
        ...
        assert len(crash_findings) >= 1, (
            f"Expected crash finding, got: {[f.title for f in findings]}"
        )

    def test_crash_severity_is_critical_or_warning(
        self, plugin: CrashDetectionPlugin, vibration_crash_flight: Flight
    ) -> None:
        findings = plugin.analyze(vibration_crash_flight, {})
        crash_findings = [f for f in findings if f.title.lower().startswith("crash detected")]
    ...
    def test_crash_has_classification(
        self, plugin: CrashDetectionPlugin, vibration_crash_flight: Flight
    ) -> None:
        findings = plugin.analyze(vibration_crash_flight, {})
        crash_findings = [f for f in findings if f.title.lower().startswith("crash detected")]
        ...
        assert crash.evidence["crash_type"] in (
            "motor_failure", "power_loss", "gps_loss",
            "pilot_error", "mechanical", "unknown",
        )

    def test_crash_has_confidence(
        self, plugin: CrashDetectionPlugin, vibration_crash_flight: Flight
    ) -> None:
        findings = plugin.analyze(vibration_crash_flight, {})
        crash_findings = [f for f in findings if f.title.lower().startswith("crash detected")]
    ...
    def test_crash_has_root_cause_chain(
        self, plugin: CrashDetectionPlugin, vibration_crash_flight: Flight
    ) -> None:
        findings = plugin.analyze(vibration_crash_flight, {})
        crash_findings = [f for f in findings if f.title.lower().startswith("crash detected")]
...
class TestCrashDetectionMotorFailure:
    ...
    def test_produces_findings(
        self, plugin: CrashDetectionPlugin, motor_failure_flight: Flight
    ) -> None:
...
class TestCrashDetectionNormalFlight:
    ...
    def test_no_crash_detected(
        self, plugin: CrashDetectionPlugin, normal_flight: Flight
    ) -> None:
        findings = plugin.analyze(normal_flight, {})
        crash_findings = [f for f in findings if f.title.lower().startswith("crash detected")]
        assert len(crash_findings) == 0, (
            f"False positive crash detected: {[f.title for f in crash_findings]}"
        )

    def test_pass_finding_returned(
        self, plugin: CrashDetectionPlugin, normal_flight: Flight
    ) -> None:
        findings = plugin.analyze(normal_flight, {})
        pass_findings = [f for f in findings if f.severity == "pass"]
        assert len(pass_findings) >= 1, (
            f"Expected pass finding for normal flight, got: {[f.title for f in findings]}"
        )

    def test_pass_score_is_high(
        self, plugin: CrashDetectionPlugin, normal_flight: Flight
    ) -> None:
        findings = plugin.analyze(normal_flight, {})
        pass_findings = [f for f in findings if f.severity == "pass"]
...
class TestCrashDetectionPluginInterface:
    ...
    def test_applicable_for_manual_mode(
        self, plugin: CrashDetectionPlugin, normal_flight: Flight
    ) -> None:
