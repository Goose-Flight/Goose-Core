...
class TestMotorSaturationAnalyzeMotorFailure:
    ...
    def test_produces_findings(self, plugin: MotorSaturationPlugin, motor_failure_flight: Flight) -> None:
    ...
    def test_returns_finding_objects(self, plugin: MotorSaturationPlugin, motor_failure_flight: Flight) -> None:
        findings = plugin.analyze(motor_failure_flight, {})
        assert all(isinstance(f, Finding) for f in findings)

    def test_plugin_name_in_findings(self, plugin: MotorSaturationPlugin, motor_failure_flight: Flight) -> None:
        findings = plugin.analyze(motor_failure_flight, {})
        assert all(f.plugin_name == "motor_saturation" for f in findings)

    def test_findings_have_valid_severity(self, plugin: MotorSaturationPlugin, motor_failure_flight: Flight) -> None:
        findings = plugin.analyze(motor_failure_flight, {})
    ...
    def test_findings_have_evidence(self, plugin: MotorSaturationPlugin, motor_failure_flight: Flight) -> None:
        findings = plugin.analyze(motor_failure_flight, {})
        for finding in findings:
...
class TestMotorSaturationAnalyzeNormalFlight:
    ...
    def test_produces_findings(self, plugin: MotorSaturationPlugin, normal_flight: Flight) -> None:
        findings = plugin.analyze(normal_flight, {})
        assert len(findings) >= 1, "Motor saturation plugin should produce at least one finding"

    def test_no_critical_in_normal_flight(self, plugin: MotorSaturationPlugin, normal_flight: Flight) -> None:
        findings = plugin.analyze(normal_flight, {})
        critical_findings = [f for f in findings if f.severity == "critical"]
        assert len(critical_findings) == 0, f"Normal flight should not have critical motor saturation findings: {[f.title for f in critical_findings]}"

    def test_returns_pass_or_warning(self, plugin: MotorSaturationPlugin, normal_flight: Flight) -> None:
        findings = plugin.analyze(normal_flight, {})
...
class TestMotorSaturationPluginInterface:
    ...
    def test_applicable_for_flights(self, plugin: MotorSaturationPlugin, normal_flight: Flight) -> None:
        assert plugin.applicable(normal_flight)

    def test_analyze_accepts_config_dict(self, plugin: MotorSaturationPlugin, normal_flight: Flight) -> None:
        findings = plugin.analyze(normal_flight, {"custom_key": "value"})
        assert isinstance(findings, list)

    def test_analyze_with_empty_config(self, plugin: MotorSaturationPlugin, normal_flight: Flight) -> None:
        findings = plugin.analyze(normal_flight, {})
        assert isinstance(findings, list)
...
class TestMotorSaturationDetection:
    ...
    def test_detects_near_maximum_output(self, plugin: MotorSaturationPlugin, normal_flight: Flight) -> None:
        findings = plugin.analyze(normal_flight, {})
        assert len(findings) > 0, "Should produce findings even if no saturation detected"

    def test_reports_per_motor_analysis(self, plugin: MotorSaturationPlugin, motor_failure_flight: Flight) -> None:
        findings = plugin.analyze(motor_failure_flight, {})
        assert len(findings) > 0, "Should analyze all motors"

    def test_saturation_severity_indicates_risk(self, plugin: MotorSaturationPlugin, motor_failure_flight: Flight) -> None:
        findings = plugin.analyze(motor_failure_flight, {})
        saturation_warnings = [f for f in findings if "saturation" in f.title.lower() and f.severity in {"warning", "critical"}]
        if saturation_warnings:
            for finding in saturation_warnings:
                assert finding.severity in {"warning", "critical"}, f"Saturation warnings should be warning or critical, got: {finding.severity}"
